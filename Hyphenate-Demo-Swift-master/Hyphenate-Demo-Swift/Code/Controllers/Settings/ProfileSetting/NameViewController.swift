...
class NameViewController: UIViewController {
    ...
    @IBAction func SaveText(_ sender: UIButton) {
        ...
        } else if emptyText(nameChangTextView) {
    ...
    func textCount(_ textView: UITextField) -> Bool {
        ...
        return (numberOfChars! < 16 && numberOfChars! > 0)
    }
    ...
    // Empty Text Check
    func emptyText(_ textView: UITextField) -> Bool {
        let numberOfChars = textView.text?.characters.count
        return numberOfChars! == 0
    }
    
    func createEmptyNameAlert (){
        let alert = UIAlertController(title: "Format Error", message: "Username Cannot be Empty", preferredStyle: UIAlertControllerStyle.alert)
        alert.addAction(UIAlertAction(title:"Try Again", style: UIAlertActionStyle.default, handler:nil))
        self.present(alert, animated: true, completion: nil)
    }
